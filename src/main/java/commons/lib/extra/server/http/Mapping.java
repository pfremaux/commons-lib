...
import commons.lib.extra.server.http.handler.testLib.JsonField;
...
public class Mapping {
	...
	public static <T> T trivialJsonMappingV2(StringBuilder data, Class<T> tClass) throws NoSuchFieldException,
		...
		for (char c : data.toString().toCharArray()) {
			...
			if (c == '{') {
				...
				if (currentField == null) {
					...
					final Object subObject = trivialJsonMappingV2(
				...
				if (!isReadingObject) {
					...
					if (isInArray) {
						...
						attributes.put(currentField,
								trivialJsonMappingV2(currentValueBeingBuilt, declaredField.getType()));
						currentField = null;
						currentValueBeingBuilt.setLength(0);
						readingMode = ReadingMode.NONE;
					}
