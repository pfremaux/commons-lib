...
public class JsonMapper {
    ...
    public static <T> T jsonToObject(StringBuilder data, Class<T> tClass) throws NoSuchFieldException, InvocationTargetException, InstantiationException, IllegalAccessException {
        ...
        for (char c : data.toString().toCharArray()) {
            ...
            if (c == '{') {
                ...
                if (currentField == null) {
                    ...
                    final Object subObject = jsonToObject(
                            new StringBuilder(data.substring(i, closingCharacterIndex)), declaredField.getType());
                    if (isInArray) {
                ...
                if (!isReadingObject) {
                    ...
                    if (isInArray) {
                        ...
                        Field declaredField = currentObject.getDeclaredField(currentField);
                        attributes.put(currentField,
                                jsonToObject(currentValueBeingBuilt, declaredField.getType()));
                        currentField = null;
                        currentValueBeingBuilt.setLength(0);
                        readingMode = ReadingMode.NONE;
                    }
    ...
    public static void main(String[] args) throws NoSuchFieldException, InvocationTargetException,
        ...
        data.append("}");
        data.append("}");
        TestF result = jsonToObject(data, TestF.class);
